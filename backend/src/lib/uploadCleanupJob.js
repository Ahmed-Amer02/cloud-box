...
const cleanupExpiredPendingSessions = async () => {
  ...
  if (count > 0) {
    logger.info({ count }, '[CleanupJob] Marked expired PENDING sessions as ABORTED');
  }
};

const cleanupStuckUploadingSessions = async () => {
  const cutoff = new Date(Date.now() - STUCK_UPLOAD_TIMEOUT_MINUTES * 60 * 1000);
  ...
  for (const session of stuckSessions) {
    try {
      ...
      await deleteS3Object(session.s3Key).catch((err) => {
        logger.error({ key: session.s3Key, err }, '[CleanupJob] Failed to delete S3 object');
      });
      ...
      logger.info({ sessionId: session.id }, '[CleanupJob] Cleaned up stuck session');
    } catch (err) {
      logger.error({ sessionId: session.id, err }, '[CleanupJob] Failed to clean up session');
    }
  }
};
...
export const runUploadCleanupJob = async () => {
  ...
  try {
    ...
    logger.error({ err }, '[CleanupJob] Unexpected error during cleanup');
  }
