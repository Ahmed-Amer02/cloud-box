...
describe('updateFolder — move validation and cycle detection', () => {
  ...
  it('rejects a folder becoming its own direct parent', async () => {
    ...
    await expect(updateFolder(user.id, folder.id, { parentId: folder.id })).rejects.toMatchObject({
      status: 400,
    });
  });
  ...
  it('rejects moving a folder into its own descendant (would create a cycle)', async () => {
    ...
    await expect(updateFolder(user.id, root.id, { parentId: grandchild.id })).rejects.toMatchObject(
      { status: 400 }
    );
  ...
  it('rejects updating a folder that belongs to a different user', async () => {
    ...
    await expect(updateFolder(attacker.id, folder.id, { name: 'Renamed' })).rejects.toMatchObject({
      status: 404,
    });
  });
});
